...
def test_fetch_build_numbers_for_branch_test(mock_browser_client):
    expected_branch_name = 'branch'
    expected_branch_url = base_url + expected_branch_name

    fetch_build_information_for_branch = FetchBuildNumbersForBranch(mock_browser_client, expected_branch_name)

    assert expected_branch_name == fetch_build_information_for_branch.branch_name
    assert mock_browser_client == fetch_build_information_for_branch.browser_client
    assert expected_branch_url == fetch_build_information_for_branch.branch_url


def test_fetch_build_numbers_for_branch_execute(mock_browser_client):
    expected_build_numbers = [1, 2, 3]
    expected_branch_name = 'branch'
    expected_branch_url = base_url + expected_branch_name

    fetch_build_numbers_for_branch = FetchBuildNumbersForBranch(mock_browser_client, expected_branch_name)

    mock_find_elements_by_css_selector_return_value = [
        MagicMock(text='#1'), MagicMock(text='#2-Information'), MagicMock(text='#3')
    ]
    mock_browser_client.find_elements_by_css_selector.return_value = \
        mock_find_elements_by_css_selector_return_value
