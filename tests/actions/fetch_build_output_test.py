...
def test_fetch_build_output_init(browser_client):
    ...
    fetch_build_output = FetchBuildOutput(browser_client, expected_branch_name, expected_build_number)
...
def test_execute(browser_client):
    fetch_build_output = FetchBuildOutput(browser_client, 'branch', 1)

    build_output = fetch_build_output.execute()

    assert browser_client.find_element_by_class_name.return_value.text == build_output
    browser_client.get.assert_called_with(fetch_build_output.console_url)
    browser_client.find_element_by_class_name.assert_called_with(console_css_class)
